...
export const ReportsPage: React.FC<ReportsPageProps> = ({ 
  ...
  const handleExportCSV = () => {
    if (!survey) return;
    window.open(apiService.resolveUrl(`/api/surveys/${survey.survey_id}/csv`), '_blank');
    setDownloadMsg('Generated detection CSV export successfully.');
  };

  const handleExportGeoJSON = () => {
    if (!survey) return;
    window.open(apiService.resolveUrl(`/api/surveys/${survey.survey_id}/geojson`), '_blank');
    setDownloadMsg('Generated spatial GeoJSON export successfully.');
  };

  const handleExportSummary = () => {
    if (!survey) return;
    window.open(apiService.resolveUrl(`/api/surveys/${survey.survey_id}/summary`), '_blank');
